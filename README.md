...
// 3. Admit the agent. The Ghostbroker delegation W3C VC is the only credential
...
is a Ghostbroker-style W3C Verifiable Credential — the credential
...
`ghostbroker-delegation:<vc-id>`. Save it — you'll pass it on every
...
interface AdmitAgentRequest {
  ...
  agentDid: string;
  delegationCredential: unknown;   // Ghostbroker delegation W3C VC
}
...
| `DELEGATION_CREDENTIAL_PATH` | admit | Path to the Ghostbroker delegation W3C VC (e.g. `output/delegations/agent_delegation.json`). |
